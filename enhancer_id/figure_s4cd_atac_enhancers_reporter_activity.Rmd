...
options(knitr.figure_dir = "figure_s4cd_atac_enhancers_reporter_activity")
...
# Figure S4c and S4d: ATAC enhancers reporter activity
...
groups.list <- readRDS("figure_s4b_atac_nearest_gene_comparison/nearest_gene_groups.list.rds")
...
bymeso.df$pvalue <- bymeso.df$group_name %>%
                    lapply(term_pvalue_for_group, bymeso.df, "Non-differential ATAC") %>%
                    unlist
...
byamnio.df$pvalue <- byamnio.df$group_name %>%
                     lapply(term_pvalue_for_group, byamnio.df, "Non-differential ATAC") %>%
                     unlist
...
bymeso.df %>% select(-terms) %>% pander(caption="One-sided proportion test for mesoderm terms compared to non-differential ATAC")
byamnio.df %>% select(-terms) %>% pander(caption="One-sided proportion test for amnioserosa terms compared to non-differential ATAC")
